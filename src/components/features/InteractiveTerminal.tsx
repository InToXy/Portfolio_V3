...
const InteractiveTerminal: React.FC<InteractiveTerminalProps> = ({ onNavigate, content }) => {
                ...
                <tbody>
                  {PROJECTS.slice(0, 5).map((proj, i) => (
                    <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="py-2 px-2 text-slate-400">{proj.category}</td>
                      <td className="py-2 px-2 text-slate-500">{proj.tags.slice(0, 2).join(', ')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      case 'experience':
        output = (
          <div className="font-mono text-sm space-y-4">
            ...
            {EXPERIENCE.map((exp, i) => (
              <div key={i} className="pl-2 border-l-2 border-slate-700 ml-1">
                <div className="flex flex-col md:flex-row md:items-baseline gap-2">
                  ...
                  <span className="text-blue-400 font-bold">{exp.role}</span>
                  <span className="text-slate-500">@ {exp.company}</span>
                </div>
                <ul className="list-disc list-inside text-slate-400 mt-1 pl-2">
            ...
            <div className="text-green-400 animate-pulse text-xs mt-4">_END OF LOG</div>
          </div>
        );
        break;
  ...
  return (
    ...
    >
      ...
      <div
        ref={containerRef}
        className="flex-1 p-6 overflow-y-auto text-slate-300 space-y-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent custom-cursor-area"
      >
